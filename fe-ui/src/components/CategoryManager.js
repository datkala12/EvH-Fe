...
export default function ProductManager(props) {
    ...
    const [cate_image, setCate_image] = React.useState('')
    const [cate_cover, setCate_cover] = React.useState('')
    const [cate_des, setCate_des] = React.useState('')
    ...
    const columns = [
        { id: 'id', name: 'Id' },
        { id: 'name', name: 'Name' },
        // { id: 'cate_image', name: 'Image Menu' },
        // { id: 'cate_cover', name: 'Image Cover' },
        { id: 'cate_des', name: 'Description' }
    ]

    const addCategory = async () => {
        try {
            const res = await axios.post("http://localhost:8080/api/admin/category", {
                id, name,
                // cate_image, cate_cover, 
                cate_des
            })
            return res.data
        } catch (error) {
            ...
            <Paper sx={{ margin: '1%' }}>
                ...
                {openAdd && (
                    <Grid container spacing={2} sx={{ padding: 5 }}>
                        <Grid item xs={3}>
                            <TextField id="outlined-basic" value={id} onChange={(e) => { setId(e.target.value) }} label="Category Id" variant="outlined" fullWidth />
                        </Grid>
                        <Grid item xs={3}>
                            <TextField id="outlined-basic" value={name} onChange={(e) => { setName(e.target.value) }} label="Category name" variant="outlined" fullWidth />
                        </Grid>
                        {/* <Grid item xs={3}>
                            <TextField id="outlined-basic" value={cate_image} onChange={(e) => { setCate_image(e.target.value) }} label="Image menu" variant="outlined" fullWidth />
                        </Grid>
                        <Grid item xs={3}>
                            <TextField id="outlined-basic" value={cate_cover} onChange={(e) => { setCate_cover(e.target.value) }} label="Image cover" variant="outlined" fullWidth />
                        </Grid> */}
                        <Grid item xs={3}>
                            <TextField id="outlined-basic" value={cate_des} onChange={(e) => { setCate_des(e.target.value) }} label="Category description" variant="outlined" fullWidth />
                        </Grid>
                        <Grid item xs={3} >
                            <Button onClick={addCategory} variant="outlined"> Submit
                            </Button>
                        </Grid>
                            ...
                            <TableBody>
                                {data?.map((category) => (
                                    <TableRow>
                                        ...
                                        {/* <TableCell>{category.cate_image}</TableCell>
                                        <TableCell>{category.cate_cover}</TableCell> */}
                                        <TableCell>{category.cate_des}</TableCell>
                                    </TableRow>
                                )
                                )
                                }
